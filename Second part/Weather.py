# IMPORT libraries
from tkinter import ttk
from ttkthemes import ThemedTk
import requests
from tkinter import messagebox
import time

# SET API from server
API_KEY = '103e4d082a5022c3512615445cad5a7d'
API_URL = 'https://api.openweathermap.org/data/2.5/weather'


# DECLARATION functions
def print_weather(weather):
    try:
        city = weather['name']
        country = weather['sys']['country']
        temp = weather['main']['temp']
        press = weather['main']['pressure']
        humidity = weather['main']['humidity']
        wind = weather['wind']['speed']
        desc = weather['weather'][0]['description']
        sunrise_ts = weather['sys']['sunrise']
        sunset_ts = weather['sys']['sunset']
        sunrise_struct_time = time.localtime(sunrise_ts)
        sunset_struct_time = time.localtime(sunset_ts)
        sunrise = time.strftime('%H:%M:%S', sunrise_struct_time)
        sunset = time.strftime('%H:%M:%S', sunset_struct_time)
        return f"Местоположение: {city}, {country} \n" \
               f"Температура: {temp} °C \n" \
               f"Атм. давление: {press} гПа \n" \
               f"Влажность: {humidity}% \n" \
               f"Скорость ветра: {wind} м/с \n" \
               f"Погодные условия: {desc} \n" \
               f"Восход: {sunrise} \n" \
               f"Закат: {sunset}"
    except:
        return 'Ошибка получения данных...'


def get_weather(event=''):
    if not entry.get():
        messagebox.showwarning('Warning', 'Enter request (city, country code')
    else:
        params = {
            'appid': API_KEY,
            'q': entry.get(),
            'units': 'metric',
            'lang': 'ru'
        }
        r = requests.get(API_URL, params=params)
        weather = r.json()
        label['text'] = print_weather(weather)


# CREATE root object
root = ThemedTk(theme='arc')
root.geometry('500x400')
root.resizable(0, 0)

# ADD icon and title
root.iconbitmap('wt.ico')  # ico image
root.title('See the whether')  # title

# CHOOSE Style from TTK library
s = ttk.Style()
s.configure('TLabel', padding=5, font='Arial 11')

# CREATE top_frame
top_frame = ttk.Frame(root)
top_frame.place(relx=0.5, rely=0.1, relwidth=0.9, relheight=0.1, anchor='n')

# CREATE entry area
entry = ttk.Entry(top_frame)
entry.place(relwidth=0.7, relheight=1)
entry.bind('<Return>', get_weather)

# CREATE Button
button = ttk.Button(top_frame, text='Request weather!', command=get_weather)
button.place(relx=0.7, relwidth=0.3, relheight=1)

# CREATE lower_frame
lower_frame = ttk.Frame(root)
lower_frame.place(relx=0.5, rely=0.25, relwidth=0.9, relheight=0.6, anchor='n')

# CREATE label
label = ttk.Label(lower_frame, anchor='nw')
label.place(relwidth=1, relheight=1)

root.mainloop()
